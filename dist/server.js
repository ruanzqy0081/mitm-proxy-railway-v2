...
import { addLicense, checkLicense, renewLicense, getAllLicenses, deleteLicense, getLogs, cleanupExpiredLicenses } from './database/db.js';
dotenv.config();
const app = express();
 ...
 */
app.post('/admin/add-license', requireAdminKey, async (req, res) => {
    try {
 ...
 * Headers: X-Admin-Key: <ADMIN_KEY>
 */
app.post('/admin/renew-license', requireAdminKey, async (req, res) => {
    try {
 ...
 * Headers: X-Admin-Key: <ADMIN_KEY>
 */
app.delete('/admin/delete-license', requireAdminKey, async (req, res) => {
    try {
        const { udid } = req.query;
        if (!udid || typeof udid !== 'string') {
            return res.status(400).json({ error: 'UDID é obrigatório' });
 ...
 * Headers: X-Admin-Key: <ADMIN_KEY>
 */
app.post('/admin/cleanup', requireAdminKey, async (req, res) => {
    try {
